...
class Image_Aligner:
    ...
    def __init__(self, config_path: str):
        ...
        try:
            ...
            logger.warning(f'Source folder has {len(self.source_files)} images while target folder has {len(self.target_files)}. Some images might be missing.')
        ...
        try:
            ...
            logger.critical(f"'smaller_fov_modatlity' must be 'source' or 'target', not {self.smaller_fov_modality}")
            raise
        ...
        for transform in self.transforms:
            try:
                ...
                logger.critical(f'{transform} is not a valid option. Valid transforms are: {valid_transforms}')
                raise
    ...
    def align_images(self):
        ...
        for i, (source_file) in enumerate(tqdm(self.source_files)):
            logger.info('--------------------------------------------------------------------')
            logger.info(f"Beginning alignment of image pair {i} with filename \'{source_file}\'")
            logger.info('--------------------------------------------------------------------')
            ...
            try:
                ...
                logger.warning(f"File with name '{source_file}' not found among target files. Skipping this image pair.")
                continue

            if os.path.isfile(os.path.join(self.source_output, source_file.replace('.czi', '.tiff'))):
                logger.warning(f"File with name '{source_file.replace('.czi', '.tiff')}' already exists in output folder. Skipping this image pair.")
                continue
            
            # Load and preprocess images
            target_file = source_file
            source = np.array(AICSImage(os.path.join(self.source_path,source_file)).data).squeeze()
            target = np.array(AICSImage(os.path.join(self.target_path,target_file)).data).squeeze()

            if source.ndim == 4:
                if np.argmin(source.shape) != 0:
                    logger.debug(f'Rearranging channels in source image. Current shape is {source.shape}')
            ...
            if target.ndim == 4:
                if np.argmin(target.shape) != 0:
                    logger.debug(f'Rearranging channels in target image. Current shape is {target.shape}')
            ...
            if self.pad_z:
                ...
                source_means = [np.mean(source[c][source[c]<thresh], axis=None) for c, (thresh) in enumerate(source_bg)]
                source_stds = [np.std(source[c][source[c]<thresh], axis=None) for c, (thresh) in enumerate(source_bg)]
                target_means = [np.mean(target[c][target[c]<thresh], axis=None) for c, (thresh) in enumerate(target_bg)]
                target_stds = [np.std(target[c][target[c]<thresh], axis=None) for c, (thresh) in enumerate(target_bg)]
                
                # generate synthetic background layers as gaussian noise
                source_pad = None
                target_pad = None
                for _, (mean, std) in enumerate(zip(source_means, source_stds)):
                    temp = np.random.normal(mean, std, (1,
                                                        self.pad_size,
                                                        source.shape[2],
                                                        source.shape[3])).astype(np.uint16)
                ...
                for _, (mean, std) in enumerate(zip(target_means, target_stds)):
                    temp = np.random.normal(mean, std, (1,
                                                        self.pad_size,
                                                        target.shape[2],
                                                        target.shape[3])).astype(np.uint16)
                ...
                source_content_label = np.expand_dims(np.ones_like(source[0]),axis=0)*255
                source_label = np.concatenate((source_pad_label,source_content_label,source_pad_label), axis=1)

                target_pad_label = np.expand_dims(np.zeros_like(target_pad[0]),axis=0)
                target_content_label = np.expand_dims(np.ones_like(target[0]),axis=0)*255
                target_label = np.concatenate((target_pad_label,target_content_label,target_pad_label), axis=1)
            ...
            aligned_src, aligned_tgt, composite = alignment.perform_alignment(source,
                                                        target,
            ...
            if aligned_src is not None:
                ...
                logger.debug(f'final shape of source image is: {aligned_src.shape}')
                logger.debug(f'final shape of target image is: {aligned_tgt.shape}')
                try:
                    with writers.OmeTiffWriter(os.path.join(self.source_output, source_file), overwrite_file=True) as writer:
                        writer.save(aligned_src, dimension_order='CZYX')
                    with writers.OmeTiffWriter(os.path.join(self.target_output, target_file), overwrite_file=True) as writer:
                        writer.save(aligned_tgt, dimension_order='CZYX')
                except:
                ...
                if self.save_composite and composite is not None:
                    ...
                    skio.imsave(os.path.join(self.composite_path, source_file), composite)
                    # with writers.OmeTiffWriter(os.path.join(self.composite_path, source_file), overwrite_file=True) as writer:
    ...
    def get_image_list(self, path):
        ...
        im_files = [f for f in dir_list if (('.tif' in f) or ('.tiff' in f) or ('.czi' in f))]
        im_files.sort()
