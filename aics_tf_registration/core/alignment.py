...
def perform_alignment(source: np.ndarray,
                        target: np.ndarray,
    ...
    ------------
        ...
        - smaller_fov_modality: which modality has the smaller field of view ('source' or 'target') 
    ...
    if smaller_fov_modality == 'source':
        ...
        moving = exp.rescale_intensity(source_align, out_range=np.uint16).astype(np.uint16)
        fixed = exp.rescale_intensity(target_align, out_range=np.uint16).astype(np.uint16)
        # moving = source_align
        # fixed = target_align
        ...
        moving = exp.rescale_intensity(target_align, out_range=np.uint16).astype(np.uint16)
        fixed = exp.rescale_intensity(source_align, out_range=np.uint16).astype(np.uint16)
        # moving = target_align
        # fixed = source_align
    ...
    if init_z_padding > 0:
        ...
        moving_scaled_adjust_z = np.pad(moving_scaled, 
        ...
        moving_scaled_adjust_z = moving_scaled[int(abs(init_z_padding)//2):(moving_scaled.shape[0] - int(math.ceil(init_z_padding/2))),:,:]
    elif init_z_padding == 0:
        moving_scaled_adjust_z = moving_scaled.copy()
    logger.debug(f'Final rescaled shape of moving image is {moving_scaled_adjust_z.shape}')

    # perform initial 2d alignment
    logger.info('Beginning calculation of rigid offset in x and y')
    fixed_2dAlign_offset_x, fixed_2dAlign_offset_y = align_xy(fixed,moving_scaled_adjust_z)
    if fixed_2dAlign_offset_x is not None:
        logger.info(f'2d alignment successful')
    ...
    if buffered_offset_x < 0 or buffered_offset_y < 0:
        logger.warning('A offset from 2d-alignment is negative. Moving image is not wholly in fixed FOV. Consider using pre-cropping')
        return None, None, None

    # prepare images for alignment with itk
    moving_scaled_adjust_for_itk = np.zeros((moving_scaled_adjust_z.shape[0], 
    ...
    fixed_addition_offset_z, fixed_addition_offset_x, fixed_addition_offset_y = align_z(fixed_pre_crop_for_itk,
    ...
    fixed, moving = finalize_alignment(fixed,
                                        moving,
                                        moving_scaled.shape,
    ...
    for channel in range(moving_out.shape[0]):
        ...
        fixed_temp, moving_temp = finalize_alignment(fixed_chan,
                                                        moving_chan,
                                                        moving_scaled.shape,
                                                        moving_scaled_adjust_z.shape,
                                                        init_z_padding,
        ...
        if not (fixed_temp is None) and not (moving_crops is None):
            moving_temp = moving_temp[moving_crops[0][0]:moving_crops[0][1],
                                    moving_crops[1][0]:moving_crops[1][1],
                                    moving_crops[2][0]:moving_crops[2][1]]
            fixed_temp = fixed_temp[fixed_crops[0][0]:fixed_crops[0][1],
        ...
        if (fixed_final is None) and not (fixed_temp is None):
            ...
            fixed_final = np.concatenate((fixed_final, np.expand_dims(fixed_temp, axis=0)), axis=0)
            moving_final = np.concatenate((moving_final, np.expand_dims(moving_temp, axis=0)), axis=0)
    ...
    if save_composite:
        ...
        moving = moving[moving_crops[0][0]:moving_crops[0][1],
                                    moving_crops[1][0]:moving_crops[1][1],
                                    moving_crops[2][0]:moving_crops[2][1]]
        fixed = fixed[fixed_crops[0][0]:fixed_crops[0][1],
                                    fixed_crops[1][0]:fixed_crops[1][1],
                                    fixed_crops[2][0]:fixed_crops[2][1]]
        ...
        try:
            t1 = sitk.Cast(sitk.RescaleIntensity(sitk.GetImageFromArray(target_comp)), sitk.sitkUInt8)
            t2 = sitk.Cast(sitk.RescaleIntensity(sitk.GetImageFromArray(tf.resize(source_comp, target_comp.shape))), sitk.sitkUInt8)
            cimg = sitk.Compose(t1, t2, t1//2.+t2//2.)
            composite = np.array(sitk.GetArrayFromImage(cimg))
...
def align_xy(fixed: np.ndarray, moving: np.ndarray):
    ...
    # inf, sup = np.percentile(fixed,[10,90])
    # fixed = np.clip(fixed,inf,sup)
    # inf, sup = np.percentile(moving,[10,90])
    ...
    # fixed_proj = exp.rescale_intensity(fixed_proj,out_range=(-1,1))
    # fixed_proj = exp.equalize_adapthist(fixed_proj,clip_limit=0.1)
    fixed_proj = gaussian(fixed_proj)
    fixed_proj = median(fixed_proj)
    fixed_proj = exp.rescale_intensity(fixed_proj,out_range=(0,65535)).astype(np.uint16)

    inf, sup = np.percentile(moving_proj,[5,95])
    moving_proj = np.clip(moving_proj,inf,sup)
    # moving_proj = exp.rescale_intensity(moving_proj,out_range=(-1,1))
    # moving_proj = exp.equalize_adapthist(moving_proj,clip_limit=0.1)
    moving_proj = gaussian(moving_proj)
    moving_proj = median(moving_proj)
    moving_proj = exp.rescale_intensity(moving_proj,out_range=(0,65535)).astype(np.uint16)
    ...
    for trial_idx in range(10):
        ...
        matches = match_descriptors(descriptors_fix, descriptors_mov, metric='euclidean', max_ratio=0.95, cross_check=True)
        ...
        model_robust, _ = ransac((src,dst), tf.EuclideanTransform, min_samples=3, residual_threshold=2, max_trials=200)
        logger.debug(f'Estimated transfrom matrix:')
        for i in range(model_robust.params.shape[0]):
            logger.debug(str(model_robust.params[i,:]))

        if abs(model_robust.params[0,0] - 1)>0.1 or abs(model_robust.params[1,1]-1)>0.1 or abs(model_robust.params[0,1])>0.1 or abs(model_robust.params[1,0])>0.1:
            # if the fitted model has a lot rotatin, then there must be a problem. then try again
            logger.debug('Estimated transfrom contains too much rotation, indicating error. Retrying.')
            continue
        
        if math.isnan(model_robust.params[0,2]) or math.isnan(model_robust.params[1,2]):
            # fail
            logger.debug('Relevant transfrom parameters are NaN, indicating failure. Retrying.')
            continue
        ...
        trial_fail = False
        logger.debug(f'offset caculated by 2D alignment is y={fixed_2dAlign_offset_y}, x={fixed_2dAlign_offset_x}')
...
def align_z(fixed: np.ndarray, moving: np.ndarray, prealign: bool, denoise: bool):
    ...
    if prealign:
        ...
        moving_z_above_threshold = np.argwhere(np.max(np.max(moving_otsu, axis=1), axis=1)>0).squeeze()
        fixed_z_above_threshold = np.argwhere(np.max(np.max(fixed_otsu, axis=1), axis=1)>0).squeeze()


        moving_estimated_center_z = (np.max(moving_z_above_threshold) + np.min(moving_z_above_threshold))//2
        fixed_estimated_center_z = (np.max(fixed_z_above_threshold) + np.min(fixed_z_above_threshold))//2
    ...
    # move to itk
    fixed_itk = sitk.GetImageFromArray(exp.rescale_intensity(fixed.astype(np.float32), out_range=(0,255)).astype(np.uint8))
    fixed_itk = sitk.Cast(fixed_itk, sitk.sitkFloat32)
    moving_itk = sitk.GetImageFromArray(exp.rescale_intensity(moving.astype(np.float32), out_range=(0,255)).astype(np.uint8))
    moving_itk = sitk.Cast(moving_itk, sitk.sitkFloat32)
    
    
    #Initialize ITK-based image registration parameters
    R = sitk.ImageRegistrationMethod()
    R.SetOptimizerAsRegularStepGradientDescent(5, .01, 50)
    ...
    try:
        ...
        skio.imsave('/allen/aics/assay-dev/users/Filip/Projects/Projects/aics_alignment_module/aics-3d-alignment/moving_itk.tiff', sitk.GetArrayFromImage(moving_itk))
        skio.imsave('/allen/aics/assay-dev/users/Filip/Projects/Projects/aics_alignment_module/aics-3d-alignment/fixed_itk.tiff', sitk.GetArrayFromImage(fixed_itk))
        logger.warning(f'Alignment failed: optimizer initialization failure')
        return None, None, None
...
def finalize_alignment(fixed: np.ndarray,
    ...
    ------------
        ...
        - moving_adjust_z_shape: image dimensions of rescaled smaller FOV after adjustments to number of z stacks
        - init_z_padding: number of z stacks padded or clipped from rescaled moving image
    ...
    if init_z_padding > 0:
        ...
        if fixed_z_offset + moving_scaled_shape[0] > fixed.shape[0]:
            moving_zz_top -= int(round(abs(fixed.shape[0] - (fixed_z_offset + moving_scaled_shape[0]))/scale_factor_z))

        logger.debug(f'Moving image cropped in z from {moving_z_bot} to {moving_zz_top}')

        fixed_final = fixed[np.max([fixed_z_offset,0]):np.min([fixed_z_offset+moving_scaled_shape[0], fixed.shape[0]]),
        ...
        moving_z_bot = int(round(math.floor(abs(init_z_padding)/(2*scale_factor_z))))
        moving_zz_top = moving_orig.shape[0]-int(round(math.ceil(abs(init_z_padding)/(2*scale_factor_z))))-2

        if fixed_z_offset < 0:
            moving_z_bot += int(round(abs(fixed_z_offset)/scale_factor_z))

        if fixed_z_offset + moving_adjust_z_shape[0] > fixed.shape[0]:
            moving_zz_top -= int(round(abs(fixed.shape[0] - (fixed_z_offset + moving_adjust_z_shape[0]))/scale_factor_z))
...
def final_refinement(lr: np.ndarray,
                     hr: np.ndarray,
                     scale_factor_xy: float,
                     scale_factor_z: float,
                     min_subcrop_xy: int,
    ...
    n_xy = n_z = 1
    while n_xy*scale_factor_xy<min_subcrop_xy or abs((round(n_xy*scale_factor_xy)/n_xy)-scale_factor_xy)>error_thresh:
        n_xy += 1
    while n_z*scale_factor_z<min_subcrop_z or abs((round(n_xy*scale_factor_xy)/n_xy)-scale_factor_xy)>error_thresh:
        n_z += 1
    ...
    lr_cropped_rescaled_padded = np.pad(lr_cropped_rescaled,
                                        pad_width = ((subcrop_z,subcrop_z),
                                                     (subcrop_xy,subcrop_xy),
                                                     (subcrop_xy,subcrop_xy))
                                        )


    hr_itk = sitk.GetImageFromArray(exp.rescale_intensity(hr.astype(np.float32), out_range=(0,255)).astype(np.uint8))
    hr_itk = sitk.Cast(hr_itk, sitk.sitkFloat32)
    lr_itk = sitk.GetImageFromArray(exp.rescale_intensity(lr_cropped_rescaled_padded.astype(np.float32), out_range=(0,255)).astype(np.uint8))
    ...
    hr_z_stop = subcrop_z-hr_offset_z + int(round(lr_cropped.shape[0]*scale_factor_z))
    hr_y_start = subcrop_xy - hr_offset_y
    hr_y_stop = subcrop_xy - hr_offset_y + int(round(lr_cropped.shape[1]*scale_factor_xy))
    hr_x_start = subcrop_xy - hr_offset_x
    hr_x_stop = subcrop_xy - hr_offset_x + int(round(lr_cropped.shape[2]*scale_factor_xy))
    ...
    lr_final = lr_cropped[lr_z_start:lr_z_stop,
                          lr_y_start:lr_y_stop,
                          lr_x_start:lr_x_stop]
    hr_final = hr[hr_z_start:hr_z_stop,
                  hr_y_start:hr_y_stop,
                  hr_x_start:hr_x_stop]

    logger.debug(f'final lr shape: {lr_final.shape}')
    logger.debug(f'final hr shape: {hr_final.shape}')

    hr_true_shape = (int(round(lr_final.shape[0]*scale_factor_z)), int(round(lr_final.shape[1]*scale_factor_xy)), int(round(lr_final.shape[2]*scale_factor_xy)))

    logger.debug(f'effective scaling ratios are: {np.divide(np.array(hr_final.shape,dtype=np.float),np.array(lr_final.shape,dtype=np.float),dtype=np.float)}')
    # error = np.absolute(np.divide(np.array(hr_final.shape,dtype=np.float),np.array(lr_final.shape,dtype=np.float),dtype=np.float)-np.array([scale_factor_z, scale_factor_xy, scale_factor_xy]))
    error = np.array(hr_final.shape, dtype=np.int) - np.array(hr_true_shape, dtype=np.int)
    ...
    if np.any(np.absolute(error)>0):
        logger.warning('Final images sizes are incorrect, attempting to correct by adjusting hr cropping')
